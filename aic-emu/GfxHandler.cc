...
{
    std::map<uint64_t, boData_t>::iterator it;

    for (it = m_boList.begin(); it != m_boList.end(); it++)
    {
        ClearBo(it->first);
    }
    m_boList.clear();
...
{
    //Currently only BGRA format is supported.
    return 4;
}
...
#if (__DEBUG)
    printf("%s: remote_handle = 0x%p .. prime_fd = %d\n",
           __FUNCTION__, remote_handle, boData.fd);
#endif

    return GFX_OK;
}

GfxStatus GfxHandler::ClearBo(uint64_t remote_handle)
{
    GfxStatus sts = GFX_OK;
    std::string errString;
...
GfxStatus GfxHandler::Copy(boData_t* dstData, const void* srcBuf)
{
    ...
    size_t sizeToWrite = surf.width * surf.height * surf.pixelSize;
    ...
    memcpy(dstData->mmap_addr, srcBuf, sizeToWrite);
