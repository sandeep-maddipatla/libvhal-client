...
CmdHandler::CmdHandler(std::string ymlFile, std::string inputFile):
...
int CmdHandler::InitSocket(AicSocketData_t* info)
{
    //Socket Related Prep
    std::string path;
    if (info->hwc_sock.empty())
    ...
    path += std::to_string(info->session_id);
