...
int main(int argc, char** argv)
{
    ...
    CmdHandler* handler = new CmdHandler(config.ymlFileName, config.contentFileName);

    status = handler->InitYmlParser();
    CHECK_STATUS(status);

    status = handler->InitGfxSurfaceHandler();
    CHECK_STATUS(status);

    status = handler->InitSocket(&config.socketInfo);
    CHECK_STATUS(status);
